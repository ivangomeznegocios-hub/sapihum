...
export function InteractiveToolViewer({
    ...
    return (
        <div className="rounded-lg border overflow-hidden bg-white dark:bg-zinc-950">
            <div className="flex items-center justify-between px-3 py-1.5 bg-brand-blue dark:bg-brand-blue/30 border-b">
                <div className="flex items-center gap-2">
                    <Wrench className="h-3.5 w-3.5 text-brand-blue" />
                    <span className="text-xs font-medium text-brand-blue dark:text-brand-blue">{title}</span>
                </div>
                <Button
                    variant="ghost"
                    size="sm"
                    onClick={() => setIsFullscreen(true)}
                    className="h-6 px-2 text-[10px] gap-1 text-brand-blue hover:text-brand-blue"
                >
