...
import {
    ...
    getLatestActiveRelationshipForPatient,
...
export default async function MyPsychologistPage() {
    ...
    if (assignedPsychologist?.id) {
        const { count } = await (supabase
            .from('appointments') as any)
            .select('*', { count: 'exact', head: true })
            .eq('patient_id', profile.id)
            .eq('psychologist_id', assignedPsychologist.id)
            .eq('status', 'completed')
        ...
        <div className="space-y-8">
            <div>
                ...
                <p className="text-muted-foreground mt-1">
                    Informacion sobre tu profesional de salud mental
                </p>
            </div>
                    ...
                    <Card className="lg:col-span-2">
                        <CardHeader>
                            <div className="flex items-start gap-4">
                                <div className="h-20 w-20 rounded-full bg-gradient-to-br from-brand-blue to-brand-blue-hover flex items-center justify-center text-white text-2xl font-bold">
                                    {assignedPsychologist.full_name?.charAt(0) || 'P'}
                                </div>
                                <div className="flex-1">
                                    <CardTitle className="text-2xl">
                                        {assignedPsychologist.full_name || 'Psicologo'}
                                    </CardTitle>
                                    ...
                                    <div className="flex items-center gap-1 mt-2">
                                        ...
                                        <span className="text-sm text-muted-foreground">
                                            {assignedPsychologist.specialty || 'Psicologo General'}
                                        </span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="grid gap-4 md:grid-cols-2">
                                ...
                                <div className="flex items-center gap-3 p-3 bg-muted rounded-lg">
                                    ...
                                    <div>
                                        ...
                                        <p className="font-medium">{assignedPsychologist.specialty || 'General'}</p>
                                    </div>
                                </div>
                            </div>
                    ...
                    <CardContent className="py-12">
                        <div className="text-center">
                            ...
                            <h2 className="text-xl font-semibold mb-2">
                                Sin psicologo asignado
                            </h2>
                            <p className="text-muted-foreground mb-6 max-w-md mx-auto">
                                Actualmente no tienes un psicologo asignado.
