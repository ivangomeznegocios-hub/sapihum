...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
    ...
    if (!event) {
        return { title: 'Grabacion no encontrada | Comunidad de Psicologia' }
    }
...
export default async function GrabacionPublicaPage({ params }: PageProps) {
    const { slug } = await params
    const event = await getPublicEventBySlug(slug)
    ...
    if (!canonicalPath.startsWith('/grabaciones/')) {
        redirect(canonicalPath)
    }

    const relatedEvents = (await getPublicCatalogEvents('grabaciones'))
