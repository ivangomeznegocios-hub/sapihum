...
export default async function NosotrosPage() {
  ...
  return (
    <div className="flex flex-1 flex-col items-center bg-background">
      <section className="relative flex w-full overflow-hidden bg-background">
        <div className="absolute inset-0 sapihum-grid-bg opacity-25" />
        <div className="absolute left-1/2 top-0 h-[540px] w-[540px] -translate-x-1/2 rounded-full bg-[#2563EB]/8 blur-[140px]" />
        ...
        <div className="relative z-10 mx-auto flex w-full max-w-7xl flex-col gap-16 px-4 py-20 sm:px-6 md:py-28 lg:px-8">
          <div className="max-w-4xl">
            ...
            <h1
              className="sapihum-fade-up text-4xl font-bold leading-[1.04] tracking-tight text-foreground sm:text-5xl md:text-6xl lg:text-7xl"
          ...
          <div className="grid gap-px border border-border/[0.08] bg-white/[0.08] md:grid-cols-3">
            {stats.map((stat) => (
              <div
                key={stat.label}
                className="bg-background/80 px-6 py-8 backdrop-blur-sm md:px-8"
              >
        ...
        <div className="mx-auto grid max-w-7xl gap-10 px-4 sm:px-6 lg:grid-cols-[0.9fr_1.1fr] lg:px-8">
          ...
          <div className="space-y-6">
            {EVOLUTION_STEPS.map((step) => (
              <div
                key={step.label}
                className="border border-border/[0.08] bg-white/[0.02] p-6 transition-colors duration-300 hover:border-[#2563EB]/20"
              >
                <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#2563EB]">
                  {step.label}
                </p>
      ...
      <section className="w-full border-y border-border/[0.06] bg-background py-24 md:py-28">
        <div className="mx-auto grid max-w-7xl gap-6 px-4 sm:px-6 lg:grid-cols-2 lg:px-8">
          <div className="relative overflow-hidden border border-border/[0.08] bg-background p-8 md:p-10">
            <div className="absolute -right-8 -top-10 text-8xl font-serif text-[#2563EB]/10">
              “
            </div>
            <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#2563EB]">
              Misión
            </p>
            <p className="mt-5 text-2xl font-medium leading-relaxed text-foreground md:text-3xl">
          ...
          <div className="relative overflow-hidden border border-border/[0.08] bg-[#0b0b0b] p-8 md:p-10">
            <div className="absolute -right-8 -top-10 text-8xl font-serif text-[#2563EB]/10">
              “
            </div>
            <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#2563EB]">
      ...
      <section className="w-full bg-background py-24 md:py-28">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid gap-px border border-border/[0.06] bg-white/[0.06] md:grid-cols-2 xl:grid-cols-3">
            {ECOSYSTEM_PILLARS.map((pillar) => {
              const Icon = pillar.icon

              return (
                <div key={pillar.title} className="bg-background p-7 md:p-8">
                  <div className="inline-flex h-12 w-12 items-center justify-center rounded-full border border-[#2563EB]/20 bg-[#2563EB]/8 text-[#2563EB]">
              ...
              {featuredSpeakers.map((speaker) => {
                ...
                return (
                  <Link
                    ...
                    className="group overflow-hidden border border-border/[0.08] bg-background transition-all duration-500 hover:border-[#2563EB]/25"
                  >
                    <div className="relative aspect-[4/4.4] overflow-hidden bg-gradient-to-br from-background via-[#111] to-[#2b210a]">
                      {image ? (
                        <Image
                          src={image}
                          alt={name}
                        ...
                        <div className="absolute inset-0 flex items-center justify-center">
                          <span className="font-serif text-6xl italic text-foreground/15">
                            {name.charAt(0)}
                          </span>
                        </div>
                      )}
        ...
        <div className="mx-auto grid max-w-7xl gap-12 px-4 sm:px-6 lg:grid-cols-[1.1fr_0.9fr] lg:px-8">
          ...
          <div className="space-y-4">
            {PRINCIPLES.map((principle, index) => (
              <div
                key={principle}
                className="flex gap-5 border border-border/[0.08] bg-white/[0.02] p-5"
              >
                <div className="text-3xl font-serif text-[#2563EB]/35">
                  0{index + 1}
                </div>
                <p className="pt-1 text-lg leading-relaxed text-foreground/90">
                  {principle}
                </p>
              </div>
            ))}
      ...
      <section className="w-full border-t border-border/[0.06] bg-background py-28 md:py-36">
